...
class Learning_Region_Proposal_Network(nn.Module):
    
    def __init__(self, is_train, **kargs):
        super().__init__()
        self.backbone = TinyBackbone()
        self.anchors = TinyAnchor().anchor
        self.is_train = is_train
        self.rpn = TinyRPN(self.anchors, self.is_train)
    
    def forward(self, x, GTs=None):
        feature_map = self.backbone(x)
        rpn_output = self.rpn(feature_map, GTs)
        return rpn_output
...
if __name__ == "__main__":
    """"For test"""
    image = torch.randn((1, 3, 800, 1280)).cuda()
    model = Learning_Region_Proposal_Network(is_train=False)
    model.to('cuda')
    output_rpn = model(image)