from faster_rcnn import TinyFasterRcnn
...
def train_net(data_path, max_epoch = 300):
    ...
    model = TinyFasterRcnn(TinyAnchor('rpn/1280_800_default_anchors.npy').anchor, True, 4)
    model.to('cuda')
...
def inference(data_path, score_thresh=0.8):
    model = TinyFasterRcnn(TinyAnchor('rpn/1280_800_default_anchors.npy').anchor, False, 4)
    ...
    for file in os.listdir(data_path):
        ...
        boxes, scores, classes = outputs[0].cpu().numpy(), outputs[1].cpu().numpy(), outputs[2].cpu().numpy()

        keep = scores>score_thresh
        boxes, scores, classes=boxes[keep], scores[keep], classes[keep]

        out_name = os.path.join('results', file[:-4]+'.jpg')
        drawDetection(img_, out_name, boxes, scores=scores, class_ids=classes, class_names = ['Apple','Banana','Cocona'])     
...
if __name__ == "__main__":
    ...
    train_net(data_path, max_epoch = 200)
